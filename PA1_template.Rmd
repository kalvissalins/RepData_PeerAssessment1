...
```{r}
activityImputed$day<-as.numeric(factor(weekdays(strptime(activityImputed$date,format="%Y-%m-%d"))))
days<-function(x){
     w<-logical(length(x))
     for(i in seq(x)){
         if(x[i]==7){
...
par(mfrow=c(2,1))
plot(b,type="l",xlab="Intervals",ylab="Average steps",col="red",main="Weekends")
plot(d,type="l",xlab="Intervals",ylab="Average steps",main="Weekdays", col="blue")
```
